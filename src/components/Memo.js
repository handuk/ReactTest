...
class Memo extends React.Component {
      ...
      componentDidMount() {
          ...
          //<TimeAgo date={this.props.data.date.created}/> timeago 에러남.
      };
      //<!-- <a className="username">{this.props.data.writer}</a> wrote a log · {this.props.data.date.created} -->

    render() {
      const { data, ownership } = this.props;
      // EDITED info
      let editedInfo = (
          <span style={{color: '#AAB5BC'}}> · Edited {this.props.data.date.edited}</span>
      );
      let starStyle = (this.props.data.starred.indexOf(this.props.currentUser) > -1) ? { color: '#ff9980' } : {} ;
        ...
        const memoView = (
                <div className="card">
                  <div className="info">
                    <Link to={`/wall/${this.props.data.writer}`} className="username">{this.props.data.writer}</Link> wrote a log · {this.props.data.date.created}
